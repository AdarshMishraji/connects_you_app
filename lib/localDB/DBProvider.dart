...
import 'package:connects_you/config/DBConfig.dart';
...
// on version diff -1, this function would be called by onDowngrade
  static Future<void> _defineTables(Database db, _, [__]) async {
    final isUsersTableCreated =
        await DBProvider.createTable(TableNames.users, db);
    debugPrint(isUsersTableCreated
        ? 'user table created'
        : 'user table has not created');
    final isSharedKeysTableCreated =
        await DBProvider.createTable(TableNames.sharedKeys, db);
    ...
    final isLocalEncryptedStorageTableCreated =
        await DBProvider.createTable(TableNames.localEncryptedStorage, db);
    ...
    if (isUsersTableCreated) {
      final isRoomsTableCreated =
          await DBProvider.createTable(TableNames.rooms, db);
      debugPrint(isRoomsTableCreated
          ? 'rooms table created'
          : 'rooms table has not created');
      if (isRoomsTableCreated) {
        final isRoomUsersTableCreated =
            await DBProvider.createTable(TableNames.roomUsers, db);
        ...
        final isMessageThreadsTableCreated =
            await DBProvider.createTable(TableNames.messageThreads, db);
        ...
        if (isMessageThreadsTableCreated) {
          final isMessagesTableCreated =
              await DBProvider.createTable(TableNames.messages, db);
  ...
  static Future<bool> createTable(String tableName, [Database? paramDB]) async {
    try {
