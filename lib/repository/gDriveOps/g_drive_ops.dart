...
import 'package:connects_you/data/gDriveOps/g_drive_ops.dart';
...
class GDriveOpsRepository {
  GDriveOpsRepository._();

  static final GDriveOpsRepository _instance = GDriveOpsRepository._();

  factory GDriveOpsRepository() {
    return _instance;
  }

  final GDriveOpsDataSource gDriveOpsDataSource = GDriveOpsDataSource();
    ...
    try {
      ...
      if (shouldDecryptAndInArray) {
        if (fileResponse != null && fileResponse.isNotEmpty) {
          final Map<String, dynamic> duetKeys = fileResponse[RoomType.DUET];
          final Map<String, dynamic> groupKeys = fileResponse[RoomType.GROUP];
          final List<SharedKey> dataToSend = [];
          final aes = AesGcmEncryption(secretKey: Keys.ENCRYPTION_KEY);
  ...
  Future<bool> saveSharedKeysToDrive(List<SharedKey> sharedKeys) async {
    try {
      ...
      AesGcmEncryption aesGcm =
          AesGcmEncryption(secretKey: Keys.ENCRYPTION_KEY);
